...
int check_double(int is, double val)
{
	if (is != 1)
	{
		fprintf(stderr, "Input error\n");
...
void *work(double *mas, int *length, int number_array)
{
    ...
    if (decision == 1)
    {
        ...
        int is_norm_input = scanf("%d", length);

        if (check_int(is_norm_input, *length, 1, 10000) == -1)
        {
        	free(mas);
        	return NULL;
        }
    ...
    else if (decision == 2)
    {
        ...
        int is_norm_input1 = scanf("%d", &position);

        if (check_int(is_norm_input1, position, 1, 10000) == -1)
        {
        	free(mas);
        	return NULL;
        }
        ...
        printf("Enter value: ");
        int is_norm_input2 = scanf("%lf", &value);

        if (check_double(is_norm_input2, value) == -1)
        {
            free(mas);
        	return NULL;
        }
    ...
    else if (decision == 3)
    {
        ...
        printf("Enter position: ");
        int is_norm_input = scanf("%d", &position);

        if (check_int(is_norm_input, position, 1, 10000) == -1)
        {
        	free(mas);
            return NULL;
        }
...
int start_0()
{
    ...
    if (check_int(is_norm_input, decision, 1, 4) == -1) return -1;
